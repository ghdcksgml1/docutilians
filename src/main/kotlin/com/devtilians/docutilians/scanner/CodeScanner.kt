...
class CodeScanner(
    ...
    companion object {
        ...
        private val ENDPOINT_REGEX =
            mapOf(
                ...
                "ts" to
                    Regex(
                        """(@(Get|Post|Put|Delete|Patch)\s*\()|(\.(?:get|post|put|delete|patch)\s*\(\s*["'/])"""
                    ),
                "js" to Regex("""\.(?:get|post|put|delete|patch)\s*\(\s*["'/]"""),
                "py" to
                    Regex("""@(app|router)\.(get|post|put|delete|patch)\s*\(|path\s*\(\s*["']"""),
                "go" to
                    Regex(
                        """(\.(?:Get|Post|Put|Delete|Patch|HandleFunc)\s*\(\s*["'])|(http\.Handle)"""
                    ),
            )
        ...
        private val COMMENT_REGEX =
            mapOf(
                "kt" to
                    listOf(Regex("""//.*$""", RegexOption.MULTILINE), Regex("""/\*[\s\S]*?\*/""")),
                "java" to
                    listOf(Regex("""//.*$""", RegexOption.MULTILINE), Regex("""/\*[\s\S]*?\*/""")),
                "ts" to
                    listOf(Regex("""//.*$""", RegexOption.MULTILINE), Regex("""/\*[\s\S]*?\*/""")),
    ...
    private fun removeComments(content: String, extension: String): String {
        ...
        var result = content
        commentPatterns.forEach { pattern -> result = result.replace(pattern, "") }
        return result
    }
